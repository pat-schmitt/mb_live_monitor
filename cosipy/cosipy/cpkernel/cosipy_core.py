...
from mb_live_monitor.cosipy.cosipy.config import Config
from mb_live_monitor.cosipy.cosipy.constants import Constants
from mb_live_monitor.cosipy.cosipy.cpkernel.init import init_snowpack, load_snowpack
from mb_live_monitor.cosipy.cosipy.cpkernel.io import IOClass
from mb_live_monitor.cosipy.cosipy.modules.albedo import updateAlbedo
from mb_live_monitor.cosipy.cosipy.modules.densification import densification
from mb_live_monitor.cosipy.cosipy.modules.evaluation import evaluate
from mb_live_monitor.cosipy.cosipy.modules.heatEquation import solveHeatEquation
from mb_live_monitor.cosipy.cosipy.modules.penetratingRadiation import penetrating_radiation
from mb_live_monitor.cosipy.cosipy.modules.percolation import percolation
from mb_live_monitor.cosipy.cosipy.modules.refreezing import refreezing
from mb_live_monitor.cosipy.cosipy.modules.roughness import updateRoughness
from mb_live_monitor.cosipy.cosipy.modules.surfaceTemperature import update_surface_temperature
...
def cosipy_core(DATA, indY, indX, GRID_RESTART=None, stake_names=None, stake_data=None):
    ...
    for t in np.arange(nt):
        ...
        if (SNOWF is not None) and (RRR is not None):
            SNOWFALL = SNOWF[t]*(water_density/density_fresh_snow)/1000.0         #input in mm.w.e
            RAIN = RRR[t]-SNOWFALL*(density_fresh_snow/water_density) * 1000.0          #affe
        elif SNOWF is not None:
            SNOWFALL = SNOWF[t]*(water_density/density_fresh_snow)/1000.0 
        ...
        surface_mass_balance = (
            SNOWFALL * (density_fresh_snow / water_density)         #affe
            - melt
            + sublimation
            + deposition
            + evaporation
        ...
        if not WRF_X_CSPY:
            _RAIN[t] = RAIN
            _SNOWFALL[t] = SNOWFALL * (density_fresh_snow/water_density)        #affe
