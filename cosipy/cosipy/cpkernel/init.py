...
from mb_live_monitor.cosipy.cosipy.cpkernel.grid import Grid
...
def load_snowpack(GRID_RESTART):
    ...
    new_snow_height = np.float64(GRID_RESTART.NEWSNOWHEIGHT.values)
    new_snow_timestamp = np.float64(GRID_RESTART.NEWSNOWTIMESTAMP.values)
    old_snow_timestamp = np.float64(GRID_RESTART.OLDSNOWTIMESTAMP.values)
